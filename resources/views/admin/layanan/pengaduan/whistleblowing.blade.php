<!-- resources/views/admin/layanan/pengaduan/whistleblowing.blade.php -->
...
<div class="mb-6 flex justify-between items-center">
    <h1 class="text-2xl font-bold text-gray-900">Whistle Blowing System</h1>
    <div class="flex space-x-3">
        <a href="{{ route('admin.layanan.pengaduan.whistleblowing.export') }}" 
...
<div class="bg-white rounded-lg shadow">
    <div class="px-6 py-4 border-b border-gray-200">
        <div class="flex justify-between items-center">
            <h3 class="text-lg font-semibold text-gray-900">Data Whistle Blowing</h3>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Lokasi Kejadian</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tanggal Kejadian</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Uraian Pengaduan</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                        ...
                        @if($item->jenis_pelanggaran_lainnya)
                            <br><span class="text-xs text-gray-500">({{ $item->jenis_pelanggaran_lainnya }})</span>
                        @endif
                    </td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {{ $item->lokasi_kejadian }}, {{ $item->kota_kabupaten }}
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {{ \Carbon\Carbon::parse($item->tanggal_kejadian)->format('d/m/Y') }}
                        @if($item->waktu_kejadian)
                            <br><span class="text-xs">{{ $item->waktu_kejadian }}</span>
                        @endif
                    </td>
                    <td class="px-6 py-4 text-sm text-gray-500 max-w-xs">
                        <div class="truncate" title="{{ $item->uraian_pengaduan }}">
                            {{ Str::limit($item->uraian_pengaduan, 100) }}
                        </div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {{ $item->email ?? '-' }}
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                        <a href="{{ route('admin.layanan.pengaduan.whistleblowing.edit', $item->id) }}" 
                           class="text-blue-600 hover:text-blue-900 inline-flex items-center">
                           <i class="fas fa-edit mr-1"></i>Edit
                        </a>
                        <form action="{{ route('admin.layanan.pengaduan.whistleblowing.destroy', $item->id) }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit" 
                ...
                <tr>
                    <td colspan="9" class="px-6 py-4 text-center text-sm text-gray-500">
                        <i class="fas fa-inbox mr-2"></i>Tidak ada data whistle blowing.
                    </td>
                </tr>
                @endforelse
            </tbody>
